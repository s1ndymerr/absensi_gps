...
<div class="siswa-page-wrapper">
    ...
    <div class="page-header-section">
        <div class="header-text">
            <h1 class="main-title">Data Siswa</h1>
            <p class="sub-title">Kelola seluruh data siswa, kelas, dan status akademik.</p>
        </div>
        <div class="header-action">
            <a href="{{ route('admin.siswa.create') }}" class="btn btn-primary-gradient">
    ...
    <!-- Stats Cards (Matches Guru Style) -->
    <div class="stats-container">
        ...
        <div class="stat-card-modern">
            ...
            <div class="stat-info-box">
               <h3>{{ $siswaBaru }}</h3>
                <p class="stat-desc">Siswa Baru</p>
            </div>
        </div>
    </div>
    ...
    <div class="main-content-card">
        ...
        <!-- Toolbar (Search & Filter) - Sama Struktur Guru -->
        <div class="toolbar-section">
            <div class="search-wrapper">
                <i class="fas fa-search search-icon"></i>
                <!-- LOGIKA SISWA: Placeholder tetap cari NIS/Kelas -->
                <input type="text" placeholder="Cari nama, NIS, atau kelas..." id="searchInput" class="search-input-modern">
            </div>
            <div class="filter-group">
                <!-- LOGIKA SISWA: Filter berdasarkan Kelas, bukan Status -->
                <select class="custom-select-modern" id="kelasFilter">
                    <option value="">Semua Kelas</option>
                    <option value="X">Kelas X</option>
                    <option value="XI">Kelas XI</option>
                    <option value="XII">Kelas XII</option>
                </select>
                <button class="btn btn-outline-soft" onclick="resetFilters()">
        ...
        <!-- Data Table -->
        @if(count($siswas) > 0)
            <!-- Container Table: Margin disesuaikan agar nempel pagination (Style Guru) -->
            <div class="table-container-modern">
                <table class="data-table-modern" id="siswaTable">
                    <thead>
                        <tr>
                            <!-- Ganti ID Guru menjadi No (Siswa) -->
                            <th width="80">No</th>
...
<td class="text-id">
    {{ $siswas->firstItem() + $loop->index }}
                            ...
                            <td class="email-text">
                                <i class="fas fa-envelope text-muted-icon"></i> {{ $siswa->email }}
                            </td>
                            <td>
                                <span class="status-pill {{ $siswa->status_akun === 'aktif' ? 'pill-success' : 'pill-danger' }}">
                                    {{ $siswa->status_akun === 'aktif' ? 'Aktif' : 'Tidak Aktif' }}
                                </span>
                            </td>
                            <td>
                                <div class="action-group">
                                    <a href="{{ route('admin.siswa.edit', $siswa->id) }}" class="btn-icon-modern btn-edit-soft" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <form action="{{ route('admin.siswa.destroy', $siswa->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-icon-modern btn-delete-soft" title="Hapus" onclick="return confirm('Apakah Anda yakin ingin menghapus siswa ini?');">
            ...
            <!-- Pagination (Style Guru: Sejajar Tabel) -->
            <div class="pagination-modern">
                <div class="page-info-text">
<div class="page-info-text">
    Menampilkan 
    <strong>{{ $siswas->firstItem() }}</strong>
    –
    <strong>{{ $siswas->lastItem() }}</strong>
    dari 
    <strong>{{ $siswas->total() }}</strong>
    data siswa
</div>
                </div>
                <div class="pagination-links">
                    <!-- Menggunakan view pagination bootstrap-4 agar CSS ter-target dengan rapi -->
        ...
        @else
            ...
            <div class="empty-state-modern">
                ...
                <p>Belum ada siswa yang terdaftar. Silakan tambahkan data siswa untuk memulai.</p>
                <a href="{{ route('admin.siswa.create') }}" class="btn btn-primary-gradient">
...
<style>
    /* =========================================
       CSS GURU (DIADAPTASI UNTUK SISWA)
       ========================================= */
    :root {
        /* Warna Identik dengan Data Guru */
        --primary-soft: #6366f1;
        --bg-body: #f1f5f9;
        --text-main: #1e293b;
        --text-muted: #64748b;
    ...
    .email-text { color: var(--text-muted); font-size: 0.9rem; }
    ...
    /* =========================================
       PERBAIKAN PAGINATION (1 2 3) - SEJAJAR TABEL (Style Guru)
       ========================================= */
...
<script>
    // LOGIKA SISWA: Filter Berdasarkan Nama/Email/Kelas
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterTable() {
            ...
            tableRows.forEach(row => {
                ...
                // Match Search (Nama/Email/Kelas) AND Match Dropdown
                const matchSearch = name.includes(term) || email.includes(term) || rowKelas.includes(term);
                const matchKelas = kelasValue === "" || rowKelas.includes(kelasValue);
    ...
    function resetFilters() {
        ...
        // Trigger event manual agar terfilter ulang
